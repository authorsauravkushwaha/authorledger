...
- **Download a full backup** of your ledger (the raw SQLite file, safely snapshotted) with one click from the sidebar — because a compliance record you can't get back out isn't much of one.
...
No software is "unhackable," and anyone who tells you otherwise is selling something. What's actually true here:

- **Binds to `127.0.0.1` only.** AuthorLedger is never exposed to your network — only your own machine can reach it.
- **Zero third-party dependencies** for the core app, which also means zero third-party supply-chain risk. The optional Claude integration uses `urllib` from the standard library, not a package that could be compromised upstream.
- **Every SQL query is parameterized.** No string-built SQL anywhere in the codebase.
- **Path traversal is checked twice**: URL-decoded, resolved, and verified to still be inside `web/` before a static file is ever served.
- **No caching, anywhere.** Every response — API and static file alike — is sent with `Cache-Control: no-store`. (This one exists because of a real bug: an early version of the CSS shipped with a mistake that made a modal render open and empty on every page load; a browser had cached the broken CSS and kept showing it even after the fix landed. If anything here ever looks stale after an update, a normal refresh is now guaranteed to show the real, current version — no cache to clear.)
- **Server errors never leak internals.** A client gets a plain "Unexpected server error," never a stack trace or file path; full details go to your own console only.
- **Request bodies are capped at 10MB** and safely drained rather than causing a broken connection if something oversized comes in.
- **Backups are taken safely.** The backup endpoint uses SQLite's own online-backup API, not a raw file copy, so you can't end up with a corrupted snapshot if you back up while writing.

If you find a real security issue, please open an issue (or, if it's sensitive, reach out before disclosing it publicly).

## Verifying it actually works

Beyond the 40+ unit and integration tests (`pytest tests/`), there's an optional, real browser test that starts the actual app and clicks through it with headless Chromium — the same way a person would, not a simulation of one:
...
It checks the full workflow end to end (create a book, add and classify a chapter, run the pattern report, open both modals, download a backup, delete the book) and fails loudly on any JavaScript console error. This is optional — it's a heavier dependency than the rest of the project needs, which is exactly why it isn't required to just run AuthorLedger.
...
├── e2e_verify.py               # optional real-browser end-to-end check (needs playwright)
├── authorledger/
