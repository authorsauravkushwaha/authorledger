...
def test_backup_endpoint_returns_a_valid_sqlite_file(running_server, tmp_path):
    _, book = request(f"{running_server}/api/books", "POST", {"title": "Backup Test"})
    req = urllib.request.Request(f"{running_server}/api/backup")
    with urllib.request.urlopen(req, timeout=5) as resp:
        assert resp.status == 200
        assert "sqlite" in resp.headers.get("Content-Type", "")
    ...
    row = conn.execute("SELECT title FROM books WHERE id = ?", (book["id"],)).fetchone()
    conn.close()
    assert row["title"] == "Backup Test"
