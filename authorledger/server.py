...
ROUTES: list[tuple[re.Pattern, str, str]] = [
    ...
    (re.compile(r"^/api/backup$"), "GET", "handle_backup"),
...
def make_handler(db_path: str):
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _security_headers(self) -> None:
            ...
            # Also disables caching for every response, API and static
            # alike. This app changes its own HTML/CSS/JS between
            # versions, and a browser silently serving a stale, cached
            # style.css after you've fixed a bug is a real, confusing
            # failure mode -- worth the (tiny, local-app) cost of always
            # refetching.
        ...
        def handle_backup(self, match, body):
            # Uses SQLite's own online-backup API rather than just reading
            # the .db file's raw bytes: copying a live database file byte
            # for byte while a write might be mid-flight can hand you a
            # torn, corrupt copy. conn.backup() takes a consistent
            # snapshot regardless of what else is happening.
            conn = self._connect()
            ...
            self.send_header("Content-Disposition", 'attachment; filename="authorledger-backup.db"')
            self.send_header("Content-Length", str(len(data)))
            self._security_headers()
            self.end_headers()
            self.wfile.write(data)
